...
export class ComplaintListComponent implements OnInit {
  ...
  getComplaints() {
    ...
    this.http.get<any[]>('http://localhost:3000/complaints', { params: params })
    .subscribe(data => {
      this.complaints = data;
      this.complaintPlaces = Array.from(new Set(data.map(complaint => complaint.place)));
      this.complaintTypes = Array.from(new Set(data.map(complaint => JSON.stringify(complaint.type))))
    .map(typeString => JSON.parse(typeString));
      this.complaintSituation = Array.from(new Set(data.map(complaint => JSON.stringify(complaint.situation))))
